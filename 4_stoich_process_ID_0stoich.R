...
library(GGally)
library(broom)
source("./Functions/Isotopic_distribution_BRAIN.R")


# Data import -------------------------------------------------------------

data <- read.csv("./SpectronautRaw/20200106_Puglielli_ATaseKO_stoichiometry_Report.csv")

# Spectral library with isotope labels
spec_lib_isotope <- read.csv("./Spectral_Library/ATaseKO_complete_MQ_library_inflated_LABEL.csv")

# Human Fasta file
hs_fasta <- read.fasta(file = "./FASTA/mouse_swissprot_canonical_iRT_20180220.fasta", seqtype = "AA")
...
# Time point 
data$genotype <- as.character(unlist(lapply(data$R.FileName, function(x){
  unlist(strsplit(x, split = "_"))[3]
})))

# Replicate
data$replicate <- as.numeric(unlist(lapply(data$R.FileName, function(x){
...
spec_lib_merge <- spec_lib_isotope2 %>% 
  select(FG.Charge, PEP.StrippedSequence, FG.Id, k_count, 
         EG.ModifiedPeptide, fragment_ion_sequence, 
         F.FrgLossType, F.Charge, isotope_label)
...
data <- data %>% 
  arrange(PEP.StrippedSequence, fragment_ion_sequence, genotype, replicate)
...
length(unique(data$PG.ProteinGroups))
...
data_1k <- data_1k %>% 
  select(genotype, replicate, 
         PG.Genes, PG.ProteinDescriptions, PG.ProteinGroups, 
  ...
  arrange(genotype, replicate, PEP.StrippedSequence, -FG.ShapeQualityScore)
...
data_1k <- data_1k %>% 
  select(genotype, replicate, PG.ProteinGroups, PG.ProteinDescriptions, PG.Genes, 
         PEP.StrippedSequence, EG.ModifiedPeptide, k_count, k_site, 
         everything()) %>%
  select(-Fasta.header, -ProteinSequence) %>%
  arrange(PEP.StrippedSequence, replicate, genotype)
...
pep_sum <- frg_stoich %>% 
  group_by(genotype, replicate, PG.ProteinGroups, PG.ProteinDescriptions, PG.Genes, 
           PEP.StrippedSequence, EG.ModifiedPeptide, k_count, k_site) %>% 
  ...
  ungroup() %>% 
  arrange(PG.ProteinGroups, k_site, genotype, replicate)


# Writing tables ----------------------------------------------------------

# write.table(frg_stoich, file = "./Cleaned/20200106_Puglielli_ATaseKO_tc_tidy_frag_stoich_output.csv", sep = ",", row.names = FALSE)
# write.table(pep_sum, file = "./Cleaned/20200106_Puglielli_ATaseKO_tc_tidy_peptide_stoich_output.csv", sep = ",", row.names = FALSE)

###############
pep_sum$L_sum[pep_sum$L_sum == 0] <- NA
pep_sum$H_sum[pep_sum$H_sum == 0] <- NA
pep_sum$H_corr_sum[pep_sum$H_corr_sum == 0] <- NA
pep_sum$H_corr_sum[pep_sum$stoich_obs == 0] <- NA
pep_sum$H_corr_sum[is.nan(pep_sum$stoich_corr == 0)] <- NA

ggplot(pep_sum, aes(genotype, -log10(H_sum))) +
  geom_boxplot()

options(scipen = 999)
pep_sum_small <- pep_sum %>% subset(EG.Qvalue_H_rms <= 0.01)
quantile <- tidy(quantile(pep_sum_small$H_sum, na.rm = TRUE))
rm(test)
#pep_sum_filter$H_sum[pep_sum_filter$H_sum <= 5977] <- NA

summary <- pep_sum_small %>% 
  group_by(genotype, PG.ProteinGroups, PG.ProteinDescriptions, PG.Genes, 
           k_site, k_count, PEP.StrippedSequence, EG.ModifiedPeptide) %>% 
  summarise(n_Hsum_include = length(which(H_sum > quantile$x[which(quantile$names == "25%")])),
            n_Lsum_isnot_NA = length(which(!is.na(L_sum))),
            stoich_mean = mean(stoich_corr, na.rm = TRUE)) %>% 
  arrange(PG.ProteinGroups, k_site, genotype) %>% 
  ungroup()

summary$stoich_assesment <- NA
summary$stoich_assesment[which(summary$n_Hsum_include >= 3 & summary$n_Lsum_isnot_NA == 0)] <- "H_pep_present"
summary$stoich_assesment[which(summary$n_Hsum_include < 3 & summary$n_Lsum_isnot_NA == 0)] <- "missing_peptide"
summary$stoich_assesment[which((summary$n_Lsum_isnot_NA > 0) & (summary$stoich_mean <= 1))] <- "calc_stoich"
summary$stoich_assesment[which((summary$n_Lsum_isnot_NA > 0) & (summary$stoich_mean > 1))] <- "missing_peptide"

summary$stoich_mean <- NULL

#write.table(summary, "H_pep_present_addition.csv", sep = ",", row.names = FALSE)
