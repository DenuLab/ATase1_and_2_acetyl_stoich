...
rm(p_adj_ATase1KO, p_adj_ATase2KO, merge_ATaseKO, ms, mss, summary);gc()
...
#write.table(wide, "./Cleaned/20181219_AT1_cyto_clean_withstats.csv", sep = ",", row.names = FALSE)
...
length(which((merge_ER$Golgi == TRUE | merge_ER$ER_all == TRUE) & 
               (merge_ER$p_adj_ATase1KO <= 0.1 & merge_ER$ATase1KO_WT <= -0.05)))
length(which((merge_ER$Golgi == TRUE | merge_ER$ER_all == TRUE | merge_ER$Secreted == TRUE) & 
               (merge_ER$p_adj_ATase1KO <= 0.05 & merge_ER$ATase1KO_WT <= -0.05)))
...
length(which(merge_ER$p_adj_ATase1KO <= 0.1))
length(which(merge_ER$p_adj_ATase2KO <= 0.1))
...
ggplot(subset(merge_ER, p_adj_ATase1KO <= 0.05), aes(ATase1KO_WT)) +
  geom_density(aes(color = (ER_all == TRUE | Golgi == TRUE)))
