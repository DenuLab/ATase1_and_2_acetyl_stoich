...
# Reading in data
spec_lib <- read.csv("./Spectral_Library/20200106_Puglielli_ATase_ER_inflated.csv")

# Selecting and arranging columns
spec_lib <- spec_lib %>% 
  select(PrecursorCharge, IntModifiedPeptide, ModifiedPeptide, StrippedPeptide, LabeledPeptide,
         PrecursorMz, FragmentLossType, FragmentNumber, FragmentType, FragmentCharge,
         FragmentMz, ProteinGroups, ProteinDescription, Genes) %>% 
...
#          "PEP.StrippedSequence" = "StrippedPeptide",
...
spec_lib_isotope <- spec_lib %>%
  rename("PrecursorCharge" = "FG.Charge",
         "IntModifiedPeptide"  = "EG.ModifiedPeptide",
         "StrippedPeptide"  = "PEP.StrippedSequence",
         "FragmentLossType" = "F.FrgLossType",
         "FragmentNumber" = "F.FrgNum",
         "FragmentType" =  "F.FrgType",
         "FragmentCharge" = "F.Charge",
...
#write.table(spec_lib_isotope, file = "./Spectral_Library/ATaseKO_complete_MQ_library_inflated_LABEL.csv", sep = ",", row.names = FALSE)
